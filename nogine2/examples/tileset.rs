use nogine2::{colors::{rgba::RGBA32, Color}, graphics::{blending::BlendingMode, texture::{sprite::SpriteAtlas, Texture2D, TextureFiltering, TextureSampling, TextureWrapping}, CameraData, Graphics}, input::{keyboard::Key, Input}, math::{rect::IRect, vector2::{ivec2, uvec2, vec2}}, prelude::init_nogine2, window::{Window, WindowCfg}};
...
fn main() {
    ...
    while window.is_open() {
        window.pre_tick(CameraData { center: vec2::ZERO, extents: vec2(window.aspect_ratio(), 1.0) * 5.0 }, window.res(), RGBA32::BLACK, None);
